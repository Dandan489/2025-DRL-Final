...
def parse_args():
    ...
    parser.add_argument("--agent-model-path", type=str, default="models/MicroRTSGridModeVecEnv__ppo_bot__1__1748741523/agent.pt",
    ...
    parser.add_argument('--eval-maps', nargs='+', default=["maps/8x8/LHR2_8x8.xml"],
...
if __name__ == "__main__":
        ...
        for step in range(0, args.num_steps):
            ...
            for idx, info in enumerate(infos):
                if "episode" in info.keys():
                    if args.ai:
                        print("against", args.ai, info["microrts_stats"]["WinLossRewardFunction"], completed_episodes)
                        if info["microrts_stats"]["WinLossRewardFunction"] < 0:
                            loss_num += 1
                        elif info["microrts_stats"]["WinLossRewardFunction"] > 0:
                            win_num +=1
                        else:
                            tie_num += 1
                        ...
                        print(f"win rate: {win_num/completed_episodes}, win: {win_num}, loss: {loss_num}, tie: {tie_num}")
                    else:
                        if idx % 2 == 0:
    ...
    print("----------------------------------------------")
    print(f"Against {args.ai} {TOTAL_EPISODES}episodes")
    print(f"win rate: {win_num/TOTAL_EPISODES}, win: {win_num}, loss: {loss_num}, tie: {tie_num}")
    envs.close()
    writer.close()

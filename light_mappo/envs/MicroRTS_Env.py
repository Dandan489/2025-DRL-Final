...
class MicroRTSVecEnv(object):
    def __init__(self, args):
        ...
        self._obs_idx = [0, 1, 2, 3, 4, 10, 11, 12, 13, 18, 19, 20, 21, 22, 26, 27, 28]
        self._mask_idx = [0, 1, 5] + list(range(6, 10)) + list(range(29, 78))
        self._move_delta = [(-1, 0), (0, 1), (1, 0), (0, -1)]  # N, E, S, W
    ...
    def _obs_wrapper(self, obs):
        ...
        new_obs = np.repeat(obs[:, np.newaxis, :, :, self._obs_idx], repeats=self.num_agents, axis=1)
    ...
    def _mask_wrapper(self, mask):
        mask = mask.reshape((self.env.num_envs, self.env.height, self.env.width, -1))
        new_mask = np.zeros((self.env.num_envs, self.num_agents, self.action_space[0].n))
        for env_idx in range(self.env.num_envs):
            for agent_id, (x, y, *_) in self.agent_id_pos_map[env_idx].items():
                new_mask[env_idx, agent_id] = mask[env_idx, x, y, self._mask_idx]
        return new_mask
    ...
    def _action_wrapper(self, action):
        ...
        action_reward = np.zeros((self.env.num_envs, self.num_agents, 1)) 
        for env_idx in range(self.env.num_envs):
            for agent_id, (x, y, *_) in self.agent_id_pos_map[env_idx].items():
        ...
        for env_idx in range(self.env.num_envs):
            for agent_id in list(self.agent_id_pos_map[env_idx].keys()):
                ...
                if t < 0 and obs[env_idx, x, y, 22] == 1: # moved issued
                    ...
                    dx, dy = self._move_delta[direction]
